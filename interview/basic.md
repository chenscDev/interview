...
@BFC---基本概念:其全英文拼写为 Block Formatting Context 直译为“块级格式化上下文”。 BFC的作用是决定了元素如何对其内容进行定位，以及与其他元素的关系和相互作用。其通俗理解可以理解为一个BFC块就是一个独立的单位，其互不打扰。
@BFC---原理规则:
BFC元素垂直方向的边距会发生重叠;
...
unshift(n); 头部添加
reverse(); 反向排序,会改变原数组
sort((a,b) => return a - b); 正向排序,会改变原数组
indexOf(n); 返回某个元素在数组内的下标
forEach(); 遍历数组 没有返回值(ie6-8不兼容)
map(); 遍历数组 支持return返回(ie6-8不兼容)
slice(开始, 结束); 截取数组中的一部分返回新数组,不会修改原数组
splice(开始, 结束, 新值); 从数组中添加/删除项目，然后返回被删除的项目,会修改原数组
filter(num => return xxx ): 使用指定的函数测试所有元素，并创建一个包含所有通过测试的元素的新数组。
返回一个10到一百的随机数?
...
垃圾回收有两种实现机制:
标记清除和引用计数
标记清除是最常用的回收方式,当变量进入环境时(比如说声明一个变量),标记为进入环境,
离开环境时,将其标记为离开环境;
...
对作用域的理解?
在es5时作用域分为两种:全局作用域和局部作用域,而es6增加了块级作用域,使用let声明的变量只在当前代码块内生效;
全局作用域代表window,
局部作用域以函数划分,
...
同源:http协议;端口;域名;
前后端通信:a: Ajax--同源下的通讯;b:WebSocket--不受同源限制;c:CORS--支持跨域通信,也支持同源通信.
创建Ajax:
A:XMLHttpRequest对象的工作流程;
B:兼容性处理;
C:事件的触发条件;
D:事件的触发顺序.
...
你所了解的攻击方式?
跨站点请求伪造?
必须用户主动登录了才可以,盗用用户身份,发送恶意请求,   目前防御 CSRF 攻击主要有三种策略：验证 HTTP Referer 字段；在请求地址中添加 token 并验证；在 HTTP 头中自定义属性并验证。
Xss跨站脚本注入,是指恶意攻击者利用网站没有对用户提交数据进行转义处理或者过滤不足的缺点，进而添加一些代码，嵌入到web页面中去。使别的用户访问都会执行相应的嵌入代码。
防御方法就是对用户输入数据进行安全过滤;
错误的捕获方式?
错误分类
代码运行错误 try catch window.onerror
资源错误 节点绑定object.onerror事件 peroformance.getEntries()
...
小程序的坑？
设置数据用this.setData而不能使用this.data.xxx=xxx；
...
Cookie和sessionStrorage和LocalStortage都是保存在浏览器端；
Cookie在http请求中携带，浏览器和服务器之间来回传递。而session和local仅在本地保存；
Cookie容量小，仅为4kb，且多数浏览器一个网站最多只能保存20个cookie；可以设置过期时间；
Session和local大小可以达到5m；session为会话级别的存储，关闭当前窗口就会被销毁，local是永久储存的，除非用户主动清除缓存！	
